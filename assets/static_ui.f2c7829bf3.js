...
(function (root) {
  ...
  async function importTyped(file, kind) {
    var text = await file.text(); var obj;
  ...
  function fillEx(sel, names) { if (!sel) return; sel.innerHTML = '<option value="">— exemplos —</option>' + (names || []).map(function (n) { return '<option value="' + esc(n) + '">' + esc(n) + '</option>'; }).join(''); }
  ...
  async function injectSim() {
    ...
    var exsel = document.createElement('select'); exsel.id = 'braiExScn'; exsel.style.display = 'none'; exsel.title = 'Cenários de exemplo'; bar.appendChild(exsel);
    var exb = mkBtn('baixar exemplo', function () { downloadExample('scenario', exsel.value); }, 'Baixar um cenário de exemplo p/ depois importar'); exb.id = 'braiExScnDl'; exb.style.display = 'none'; bar.appendChild(exb);
  ...
  async function injectEditor() {
    ...
    var exsel = document.createElement('select'); exsel.id = 'braiExTree'; exsel.style.display = 'none'; exsel.title = 'Árvores de exemplo';
    var exb = mkBtn('baixar exemplo', function () { downloadExample('tree', exsel.value); }, 'Baixar uma árvore de exemplo p/ depois importar'); exb.id = 'braiExTreeDl'; exb.style.display = 'none';
